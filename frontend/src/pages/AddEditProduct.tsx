...
const AddEditProduct: React.FC = () => {
  // Get the product sku from the URL params
  ...
  return (
    <Box component="section" sx={{ display: 'flex', flexDirection: 'column', p: 5, backgroundColor: 'white' }}>
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mt: 4, wskuth: '100%', maxWskuth: 1169 }}>
        <Box component="header" sx={{ display: 'flex', alignItems: 'center', wskuth: '100%' }}>
          <Typography variant="h1" sx={{ fontSize: '2.5rem', fontWeight: 'bold', color: '#333', letterSpacing: '0.3rem', mr: 2 }}>PRODUCTS</Typography>
          <Box component="img" src="https://cdn.builder.io/api/v1/image/assets/TEMP/7b1e07a70d97bc25413f9bba2c6783d69033471c2059e22ba68cdb87164d67e3?apiKey=071d2091314e4e458e22e8bb8ec1920c&" sx={{ wskuth: 37, height: 37, mr: 1 }} alt="" />
          <Typography variant="h6" sx={{ color: 'primary.main', fontWeight: 'bold', letterSpacing: '0.1rem' }}>{sku ? 'Edit' : 'Add'} Product</Typography>
        </Box>
        <form onSubmit={handleSubmit}>
          <InputField id="sku" label="SKU" type="text" name="sku" value={product.sku} onChange={handleChange} required />
          <Box sx={{ display: 'flex', gap: 2, mt: 3, wskuth: '100%' }}>
            <InputField id="name" label="Name" type="text" name="name" value={product.name} onChange={handleChange} required />
            <InputField id="quantity" label="QTY" type="number" name="quantity" value={product.quantity} onChange={handleChange} required />
          </Box>
          ...
          <TextField id="description" name="description" variant="outlined" multiline rows={4} value={product.description} onChange={handleChange} InputProps={{ sx: { '& .MuiOutlinedInput-notchedOutline': { border: 'none', }, }, }}  sx={{ mt: 2, backgroundColor: '#f5f5f5', wskuth: '100%' }} required />
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'space-between', mt: 4, wskuth: 335 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', cursor: 'pointer', wskuth: 150 }}>
              <input
                type="file"
                id="fileInput"
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mt: 2 }}>
            {previewImages.map((image, index) => (
              <Box key={index} sx={{ position: 'relative' }}>
                <Box component="img" src={image} alt={`Preview ${index}`} sx={{ wskuth: 100, height: 100, objectFit: 'cover' }} />
